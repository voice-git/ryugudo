<?php 
    $home = esc_html(home_url('/'));
    $contact = esc_html(home_url('/contact/'));
    $privacypolicy = esc_html(home_url('/privacypolicy/'));
    $qa = esc_html(home_url('/qa/'));
    $recruit = esc_html(home_url('/recruit/'));
    $china = esc_html(home_url('/genre-china/'));
    $kutani = esc_html(home_url('/genre-kutani/'));
    $company = esc_html(home_url('/company/'));
    $reservation01 = esc_html(home_url('/reservation01/'));
    $reservation02 = esc_html(home_url('/reservation02/'));
    $kaitori = esc_html(home_url('/kaitori/'));
    $newcustomer = esc_html(home_url('/newcustomer/'));
    $antique = esc_html(home_url('/antique/'));
    $sigyo = esc_html(home_url('/sigyo/'));
    $retro = esc_html(home_url('/retro-goods/'));
    $kakejiku = esc_html(home_url('/kakejiku/'));
    $auction = esc_html(home_url('/auction/'));
    $chadougu = esc_html(home_url('/chadougu/'));
    $youshoku = esc_html(home_url('/youshoku/'));
    $gold = esc_html(home_url('/genre-gold/'));
    $kotto = esc_html(home_url('/genre-kotto/'));
    $column = esc_html(home_url('/column/'));
    $news = esc_html(home_url('/news/'));
    $buylist = esc_html(home_url('/buylist/'));
?>
<?php get_header(); ?>
    <main class="main">
      <article class="company">
        <section class="company_read">
          <div class="container-in">
            <h2>骨董品買取業者<br class="is-sp">というより、<br>
            伝統文化継承企業です。</h2>
            <p>
            ただの買取業者にはなりたくありません。<br class="is-pc">
            伝統工芸を買い叩いて売りさばくような仕事はしたくない。<br class="is-pc">
            価値あるものを、価値あるままに、<br class="is-pc">
            その価値を愛せる世界中の人のもとに届ける。<br class="is-pc">
            それが竜宮堂の責任です。<br class="is-pc">
            工芸王国と称される北陸には、<br class="is-pc">
            価値ある骨董品がたくさんあります。<br class="is-pc">
            そう、あなたのご自宅にもあるかもしれません。<br class="is-pc">
            大事なのは、そこにある価値を<br class="is-pc">
            見える化して、資産化することです。<br class="is-pc">
            そして眠らせておくのではなく、継承することです。<br class="is-pc">
            わたしたちがそのお手伝いします。<br class="is-pc">
            伝統工芸の文化を、歴史を、未来にリレーするために、<br class="is-pc">
            竜宮堂は「伝統文化継承企業」として仕事をします。
            </p>
            <img src="<?php echo get_template_directory_uri(); ?>/img/company-logo.webp" alt="ロゴ" />
          </div>
        </section>
        <section class="company_message">
          <div class="company_message_box">
            <div class="company_message_left"></div>
            <div class="company_message_right">
              <h2>TOP MESSAGE</h2>
              <p>
                弊社のホームページをご覧いただきまして誠にありがとうございます。代表者の浦嶋裕介と申します。私の座右の銘でもある不易流行（ふえきりゅうこう）」は、（変わらないものを大事にしつつ、新しい変化も取り入れる事）弊社が主に取り扱っている骨董品にも通ずるものがあります。新しいものに取り組む姿勢と伝統を守ることは、どちらが優先するものでもありません。新しいことに取り組むことも大事であり、同じく伝統を守ることも大事です。時代が変わろうとも、変わらぬ本質を大切にし、地域社会にとって必要な存在であり続けるように日々精進してまいります。
              </p>
              <div class="company_message_name">
                株式会社浦嶋<br />
                竜宮堂
              </div>
              <div class="company_message_ceo">
                <span>代表取締役社長</span><br />浦嶋 裕介
              </div>
            </div>
          </div>
        </section>
        <section class="company_info">
          <div class="container-in">
            <div class="company_info_box">
              <div class="sec_ttl_box">
                <div class="sec_ttl_logo">
                  <img src="<?php echo get_template_directory_uri(); ?>/img/chara.webp" alt="ロゴ" />
                </div>
                <h2 class="sec_ttl">会社情報</h2>
                <p class="sec_sub_ttl">COMPANY</p>
              </div>
              <div class="company_info_wrap">
                <dl>
                  <dt>会社名</dt>
                  <dd>株式会社 浦嶋</dd>
                </dl>
                <dl>
                  <dt>代表者</dt>
                  <dd>浦嶋 裕介</dd>
                </dl>
                <dl>
                  <dt>所在地</dt>
                  <dd>
                    金沢店　　　<br class="is-sp" />〒920-0052
                    <br class="is-sp" />石川県金沢市薬師堂町イ52<br />
                    福井営業所　<br class="is-sp" />〒910-0854
                    <br class="is-sp" />福井県福井市御幸4丁目13-10
                  </dd>
                </dl>
                <dl>
                  <dt>電話番号</dt>
                  <dd>
                    【代表】<br class="is-sp" />076-255-0065<br />
                    【査定依頼・お問合わせ】<br class="is-sp" />0120-947-265
                  </dd>
                </dl>
                <dl>
                  <dt>許可番号</dt>
                  <dd>
                    美術品商許可 石川県公安委員会 第511020011799号<br />
                    美術品市場許可 石川県公安委員会 第511020000077号<br />
					特定国際種事業者 象牙タイマイ等取扱業者 事業者番号 S-4-17-00053<br />
					一般酒類小売業免許取得
                  </dd>
                </dl>
              </div>
            </div>
          </div>
        </section>
        <section class="access">
          <!-- インフォ -->
          <div class="container-in">
            <!--  -->
            <div class="access_wrapper">
              <div class="sec_ttl_box">
                <div class="sec_ttl_logo">
                  <img src="<?php echo get_template_directory_uri(); ?>/img/chara.webp" alt="ロゴ" />
                </div>
                <h2 class="sec_ttl">アクセス</h2>
                <p class="sec_sub_ttl">ACCESS</p>
              </div>
              <div class="access_code">
                〒920-0052 石川県金沢市薬師堂町イ52 <br class="is-sp" /><span
                  class="margin"
                  >金沢駅から車で10分<br />
                  渋谷工業様 桜田カジマート様 <br class="is-sp" />桜田郵便局様
                  近く</span
                ><br />
                <span
                  >注：（薬師町）ではなく（薬師堂町）です。<br />
                  お間違えのないようご来店くださいませ。</span
                >
              </div>
              <div class="access_map">
                <iframe
                  src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3204.039473733857!2d136.62731837620754!3d36.57726657231172!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x5ff9ccd2686bb719%3A0xd44f4a7302a2f38b!2z56uc5a6u5aCCIOWPpOe-juihkyDpqqjokaMg5Lmd6LC354S8IOi8quWztuWhlyDjgqLjg7Pjg4bjgqPjg7zjgq8g6LK35Y-W!5e0!3m2!1sja!2sjp!4v1734935922533!5m2!1sja!2sjp"
                  style="border: 0"
                  allowfullscreen=""
                  loading="lazy"
                  referrerpolicy="no-referrer-when-downgrade"
                ></iframe>
              </div>
            </div>
          </div>
        </section>
        <!-------------- info ---------------->
        <section class="company-section-info">
            <div class="container-in">
                <div class="info">
                <div class="info_tel">
                    <a href="tel:0120-947-265">
                        <img src="<?php echo get_template_directory_uri(); ?>/img/info-tel-pc.webp" alt="電話番号" class="is-pc" />
                        <img src="<?php echo get_template_directory_uri(); ?>/img/info-tel-sp.webp" alt="電話番号" class="is-sp" />
                    </a>
                </div>
                <div class="info_btn_wrapper">
                    <a href="<?php echo $reservation01; ?>" class="info_btn">
                    <img src="<?php echo get_template_directory_uri(); ?>/img/btn-trip-info.webp" alt="出張予約" />
                    </a>
                    <a href="<?php echo $reservation02; ?>" class="info_btn">
                    <img src="<?php echo get_template_directory_uri(); ?>/img/btn-visit-info.webp" alt="来店予約" />
                    </a>
                    <a href="<?php echo $contact; ?>" class="info_btn">
                    <img src="<?php echo get_template_directory_uri(); ?>/img/btn-contact-info.webp" alt="お問い合わせ" />
                    </a>
                    <a href="https://line.me/ti/p/YD-7m1oJXw" class="info_btn" target="_blank">
                    <img src="<?php echo get_template_directory_uri(); ?>/img/btn-line.webp" alt="LINEお問い合わせ" />
                    </a>
                </div>
                <div class="info_txt">
                    <img src="<?php echo get_template_directory_uri(); ?>/img/info-txt.webp" alt="メールフォーム・LINEからのお問い合わせにはお時間をいただきます。何卒ご了承ください。" class="is-pc" />
                    <img src="<?php echo get_template_directory_uri(); ?>/img/info-txt-sp.webp" alt="メールフォーム・LINEからのお問い合わせにはお時間をいただきます。何卒ご了承ください。" class="is-sp" />
                </div>
                </div>
            </div>
        </section>
      </article>
      <!-------------- アクセス ---------------->
    </main>
<?php get_footer(); ?>
